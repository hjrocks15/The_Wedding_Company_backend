...
# Project Folder
The project is structured so that every part of the backend has a clear responsibility. The <span style="color:orange; font-weight:bold">app/main.py </span> file is the entry point of the FastAPI server and includes all API routers. Inside the <span style="color:orange; font-weight:bold">app/main.py </span>app/routers/ folder, org_router.py handles all organization-related APIs (create, get, update, delete), while <span style="color:orange; font-weight:bold">admin_router.py manages admin login.</span>. These routers do not directly work with the database; instead, they call the repository classes located in app/repositories.py, which contain all database logic such as inserting metadata, creating admin records, or managing dynamic collections. The database connection itself is defined in <span style="color:orange; font-weight:bold">app/db.py </span>, where Motor (MongoDB async driver) provides the master database and the dynamic per-organization collections. Authentication logic such as password hashing, JWT creation, and token verification is kept inside app/auth.py so routers can use it cleanly. The Pydantic models inside app/models.py define request and response schemas, ensuring validation and consistent API structure. Utility functions are placed in app/utils.py if needed. Environment variables like Mongo URI, JWT secret, and DB name are loaded from .env. Altogether, the main app receives a request → router handles route → calls service/repository → repository accesses database → response returned. This modular layout makes the system clean, scalable, and easy to extend.
